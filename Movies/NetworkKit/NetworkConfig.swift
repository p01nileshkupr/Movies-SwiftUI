...
struct EnvironmentConfig {
    private enum Keys {
        static let baseURL = "RAPIDAPI_BASE_URL"
    }
...
struct RequestConfig {
    ...
    enum Header {
        ...
        static let rapid_api_key = "X-RapidAPI-Key"
        static let rapid_api_host_key = "X-RapidAPI-Host"
        static let rapid_api_value_key = "RAPIDAPI_KEY"
        static let rapid_api_host_value_key = "RAPIDAPI_HOST"
        static let user_agent_key = "User-Agent"
        static let user_agent_value = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/75.0.3770.100 Safari/537.36"
        
        static let rapidAPIKeyValue: String = {
            guard let rapidAPIKey = Bundle.main.object(
                forInfoDictionaryKey: Header.rapid_api_value_key
            ) as? String else {
                fatalError("RAPIDAPI_KEY not found")
            }
            return rapidAPIKey
        }()
        
        static let rapidAPIHostValue: String = {
            guard let rapidAPIHost = Bundle.main.object(
                forInfoDictionaryKey: Header.rapid_api_host_value_key
            ) as? String else {
                fatalError("RAPIDAPI_HOST not found")
            }
            return rapidAPIHost
        }()
    }
